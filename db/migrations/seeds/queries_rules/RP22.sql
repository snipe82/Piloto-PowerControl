...
-- Cambios v1:
--   1. Versión inicial
--   2. La compra en curso es la cuarta — existen al menos 3 previas
--      completadas en las últimas 72 horas
...
--   7. No debe haber pasado biometría en la compra actual
--   8. No valida biometría previa — diferencia con RP21
...
WHERE fa.biometria = 'NO'
  -- Existen al menos tres compras previas completadas en los últimos 3 días
  AND (
    SELECT COUNT(*)
    ...
    WHERE fa2.customer_id = p.customer_id
      ...
      AND fa2.submission_datetime >= NOW() - INTERVAL '72 hours'
  ) >= 3
